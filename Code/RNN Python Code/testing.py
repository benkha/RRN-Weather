...
def runTesting(trainingEpoch="False", numberAirports="All"):
    ...
    #Stefan:
    if trainingEpoch != "False":
        Experiment_KEY = "TrainingEpoch_" + str(trainingEpoch) + "_" + Experiment_KEY
    #:Stefan
    ...
    #Stefan:
    hlist = []
    if numberAirports != "All":
        airports = airports[:numberAirports]
    #:Stefan
    for air_id in airports:
        ...
        if cluster_id in cluster_subset:
            ...
            a=[]
            b=[]
            #import itertools #moved import out of function
            for feat in features_trn:
                a.append(list(itertools.chain(*feat)))

            for tar in targets_trn:
                b.append(tar[-1])
            #from sklearn import linear_model #moved import out of function
            ...
            if (len(features_tst) > 0):
                for one_seq in features_tst:
                    ...
                    #from denrmalization import * #moved import out of function
    ...
    if trainingEpoch == "False":
        ...
        name = 'data/CS294Data/trainingValues' + str(trainingEpoch) + '.pkl'
        output = open(name, 'wb')
        pickle.dump(hlist, output)
        output.close()
    #:Stefan
...
def runTraining():
    ...
    a = 0
    while a < 1200:
        runTesting(trainingEpoch=a, numberAirports=numberAirports)
        a += 1
